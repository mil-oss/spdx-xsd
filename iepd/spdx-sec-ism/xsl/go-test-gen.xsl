...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exsl="http://exslt.org/common" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs" version="1.0">
    ...
    output:/iepd/src/golang/ism-struct/ism-test.go
   -->
    ...
    <xsl:template name="maketests">
        ...
        <xsl:value-of select="concat($in,$qt,'test_instance.xml',$qt,':','      ',$qt,'xml/instance/test_instance.xml',$qt,$cm,$cr,'}',$cr)"/>       
        ...
        <xsl:for-each select="/xs:schema/xs:complexType[@name = $roottype]//xs:element[@ref]">
            ...
            <xsl:for-each select="/xs:schema/xs:complexType[@name = $ty]//xs:element[@ref]">
                ...
                <xsl:for-each select="exsl:node-set($dotpath)/*">
                    ...
                    <xsl:value-of select="concat($in, $in, $in, 'Expect(', ., '.Data).To(Equal(', $qt, $testval, $qt, '))', $cr)"/>
                    <!--<xsl:value-of select="concat($in, $in, $in, 'Expect(', .,'.SecurityAttributesOptionGroup.Classification.Value', ').To(Equal(', $qt, 'U', $qt, '))', $cr)"/>-->
                </xsl:for-each>
            </xsl:for-each>
            <!--<xsl:value-of select="concat($in, $in, $in, 'Expect(','seva.',$rr ,'.SecurityAttributesOptionGroup.Classification.Value', ').To(Equal(', $qt, 'U', $qt, '))', $cr)"/>-->
