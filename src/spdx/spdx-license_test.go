...
func TestSpdxLicense(t *testing.T) {
	...
	g.Describe("SPDX", func() {
		...
		g.It("Must have StandardLicenseHeader", func() {
			Expect(spdxlic.StandardLicenseHeader).To(Equal("Standard License Template text"))
		})
		g.It("Must have StandardLicenseTemplate", func() {
			Expect(spdxlic.StandardLicenseTemplate).To(Equal("Standard License Template text"))
		})
		g.It("Must have LicenseText", func() {
			Expect(spdxlic.LicenseText).To(Equal("License text"))
		})
		...
		g.It("Must have Name", func() {
			Expect(spdxlic.Name).To(Equal("The Unlicense"))
		})
		g.It("Must have SeeAlso", func() {
			Expect(spdxlic.SeeAlso[0]).To(Equal("http://test.org/"))
		})
		g.It("Must have Comment", func() {
			Expect(spdxlic.Comment).To(Equal("License comment"))
		})
	})
